...
Refactored `MenuItem` model from multiple specific fields to a generic, polymorphic design pattern.
This follows enterprise CMS best practices for extensibility and maintainability.
...
```prisma
model MenuItem {
  type          String   // 'link', 'page', 'post', 'category', 'service', 'project', 'custom-link'
...
export enum MenuItemType {
  ...
  CUSTOM = 'custom-link', // Custom/parent item (no link)
}
```
