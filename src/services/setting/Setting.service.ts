...
export const listSettingsService = async (params: ListParams = {}) => {
...
export const getSettingByKeyService = async (key: string) => {
 ...
 if (!setting) throw Object.assign(new Error('Setting not found'), { status: 404 })
 return setting
}
...
export const createSettingService = async (data: any) => {
 const { key, value, description, group, isPublic } = data || {}
 if (!key || value === undefined) throw Object.assign(new Error('key and value are required'), { status: 422 })
...
export const updateSettingService = async (id: string, data: any) => {
 const { key, value, description, group, isPublic } = data || {}
 const payload: any = {}
